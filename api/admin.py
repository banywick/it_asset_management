...
#         df_departments = pd.DataFrame({'название': ['IT отдел', 'Бухгалтерия', 'Отдел продаж']})
...
#         df_mfp = pd.DataFrame({'модель': ['Xerox WorkCentre 123', 'HP LaserJet MFP 234']})
#         df_mfp.to_excel(writer, sheet_name='МФУ', index=False)
        
#         df_tv = pd.DataFrame({
#             'марка': ['Samsung', 'LG'],
#             'диагональ': [55, 65],
#             'номер ОС': ['ТВ-001', 'ТВ-002']
#         })
#         df_tv.to_excel(writer, sheet_name='Телевизоры', index=False)
        
#         df_monitors = pd.DataFrame({'марка': ['Dell', 'LG', 'Samsung']})
#         df_monitors.to_excel(writer, sheet_name='Мониторы', index=False)
        
#         df_ups = pd.DataFrame({
#             'модель': ['APC Back-UPS 650', 'CyberPower 1500'],
#             'номер ОС': ['ИБП-001', 'ИБП-002']
#         })
#         df_ups.to_excel(writer, sheet_name='ИБП', index=False)
        
#         df_locations = pd.DataFrame({'название': ['Минск', 'Мачулищи', 'Жодино']})
...
#                 '4. При повторной загрузке добавляются только новые записи'
#             ]
#         })
...
#     list_display = ('asset_number', 'model', 'ip_address')
...
#     list_display = ('asset_number', 'model', 'get_status_display', 'comment')
#     list_filter = ('status',)
#     search_fields = ('asset_number', 'model')
