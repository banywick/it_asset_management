    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if model and model != 'nan' and model.lower() not in ['модель', 'model']:
                ...
                if 'ремонт' in status_lower:
                    status = 'repair'
                elif 'замен' in status_lower:
                    status = 'replaced'
                else:
                    status = 'active'
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        df_instructions = pd.DataFrame({
            'Инструкция': [
                ...
                '6. Для телевизоров нужны марка, диагональ, номер ОС (опционально) и место (опционально)',
                '7. Для ИБП нужны модель, номер ОС (опционально), статус и комментарий',
...
class MFPAdmin(ImportExcelAdmin):
    ...
    search_fields = ('asset_number', 'model', 'ip_address')
    list_per_page = 20
    
    fieldsets = (
        ('Основная информация', {
            'fields': ('asset_number', 'model')
        }),
        ('Сетевые настройки', {
            'fields': ('ip_address',)
        }),
